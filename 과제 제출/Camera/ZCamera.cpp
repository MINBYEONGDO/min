...
D3DXMATRIXA16* ZCamera::SetView(D3DXVECTOR3* pvEye, D3DXVECTOR3* pvLookat, D3DXVECTOR3* pvUp)
{
	...
	m_vLookat = *pvLookat;
	m_vUp = *pvUp;
	//m_vUp = D3DXVECTOR3(-1.0f, 0.1f, 0.0f);
...
D3DXMATRIXA16* ZCamera::RotateLocalZ(float angle)
{
	...
	D3DXVECTOR3 vNewDst;
	D3DXVec3TransformCoord(&vNewDst, &m_vView, &matRot); // view * rot로 새로운 dst vector를 구한다.
	vNewDst += m_vUp;          // 실제 dst position =  eye Position + dst vector
	//vNewDst += m_vEye;
	return SetView(&m_vEye, &m_vLookat, &vNewDst);
}

D3DXMATRIXA16*	ZCamera::RotateLocalZReset()
{
	D3DXVECTOR3 a(0, 0.200000003, 0);
	//while(a != m_vUp)
	//{
		m_vUp = a;
		//m_vUp.x -= 0.1f;
