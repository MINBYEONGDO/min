...
float					g_fRot = 0.0f;
bool					g_bStart = false;
bool					g_bParentMove = false;
bool					g_bParentRotation = false;
bool					g_bChildRotation = false;
...
HRESULT InitGeometry()
{
	...
	D3DXCreateFont(g_pd3dDevice, 20, 0, FW_NORMAL, 1, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
			DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, TEXT("Arial"), &g_Font);
...
void Render()
{
	...
	g_Font->DrawText(NULL, TEXT("TEST"), -1, &g_Rect, DT_NOCLIP, D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f));
...
LRESULT WINAPI MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_SHIFT)//부모이동
		{
			if(g_bParentMove == true)
				g_bParentMove = false;
			else if (g_bParentMove == false)
				g_bParentMove = true;
		}
		if (wParam == VK_CONTROL)//부모 회전
		{
			if (g_bParentRotation == true)
				g_bParentRotation = false;
			else if (g_bParentRotation == false)
				g_bParentRotation = true;
		}
		if (wParam == VK_BACK)//자식 회전
		{
			if (g_bChildRotation == true)
				g_bChildRotation = false;
			else if (g_bChildRotation == false)
				g_bChildRotation = true;
		}
		if (wParam == VK_SPACE)
		{
			if (g_bStart == false)
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, MsgProc,
		...
		TEXT("D3D Animation"), NULL };

	RegisterClassEx(&wc);

	HWND hWnd = CreateWindow(TEXT("D3D Animation"), TEXT("D3D Animation"), WS_OVERLAPPEDWINDOW,
	...
	UnregisterClass(TEXT("D3D Animation"), wc.hInstance);
	return 0;
}