...
#define WINDOW_W		500
#define WINDOW_H		500
#define WINDOW_TITLE	TEXT("HeightMap+Camera")
#define BMP_HEIGHTMAP	TEXT("map128.bmp")
...
HRESULT InitTexture()
{
	...
	// 색깔맵
	if( FAILED( D3DXCreateTextureFromFile( g_pd3dDevice, TEXT("tile2.tga"), &g_pTexDiffuse) ) )
		return E_FAIL;
...
void SetupCamera()
{
    ...
    D3DXVECTOR3 vUpVec( 0.0f, 0.2f, 0.0f );
...
//  vecDir = D3DXVECTOR3(cosf(GetTickCount()/350.0f),	 1.0f,sinf(GetTickCount()/350.0f) );// 광원 회전
...
void ProcessMouse( void )
{
	...
	GetCursorPos( &pt );
	int dx = pt.x - g_dwMouseX;	// 마우스의 변화값 pt.x 현재 위치 , g_dwMouseX 바뀌기 전 위치 이 둘의 차의 값은 이동 거리
	...
	// 마우스를 윈도우의 중앙으로 초기화
	//SetCursor( NULL );	// 마우스 커서를 나타나지 않게 않다.
	RECT	rc;
...
{
	D3DXVECTOR3 a(0.0f, 0.200000003f, 0.0f);
	if (GetAsyncKeyState('W')) g_pCamera->MoveLocalZ(0.5f);	// 카메라 전진!
	if (GetAsyncKeyState('S')) g_pCamera->MoveLocalZ(-0.5f);	// 카메라 후진!
	if (GetAsyncKeyState('A')) g_pCamera->MoveLocalX(-0.5f);//왼
	if (GetAsyncKeyState('D')) g_pCamera->MoveLocalX(0.5f);//오
	if (GetAsyncKeyState('L')) g_pCamera->MoveLocalY(-0.5f);//아래
	if (GetAsyncKeyState('H')) g_pCamera->MoveLocalY(0.5f);//위
	if (GetAsyncKeyState('Q'))
	{
		g_pCamera->RotateLocalZ(-0.05f);//왼기울기
		D3DXMATRIXA16*	pmatView = g_pCamera->GetViewMatrix();		// 카메라 행렬을 얻는다.
		g_pd3dDevice->SetTransform(D3DTS_VIEW, pmatView);			// 카메라 행렬 셋팅
	}
	if (GetAsyncKeyState('E'))
	{
		g_pCamera->RotateLocalZ(0.05f);//오른기울기
		D3DXMATRIXA16*	pmatView = g_pCamera->GetViewMatrix();		// 카메라 행렬을 얻는다.
		g_pd3dDevice->SetTransform(D3DTS_VIEW, pmatView);			// 카메라 행렬 셋팅
	}
	if (GetAsyncKeyState('R'))//기울기 초기화
	{
		g_pCamera->RotateLocalZReset();//오른기울기
		D3DXMATRIXA16*	pmatView = g_pCamera->GetViewMatrix();		// 카메라 행렬을 얻는다.
		g_pd3dDevice->SetTransform(D3DTS_VIEW, pmatView);			// 카메라 행렬 셋팅
	}
	//else if(g_pCamera->GetUp() !=a)
	//else
	//{
	//	g_pCamera->RotateLocalZReset();
...
{
    // 윈도우 클래스 등록
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, MsgProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, TEXT("D3D Camera"), NULL };
    RegisterClassEx( &wc );

    // 윈도우 생성
    HWND hWnd = CreateWindow( TEXT("D3D Camera"), WINDOW_TITLE, WS_OVERLAPPEDWINDOW, 100, 100, WINDOW_W, WINDOW_H,GetDesktopWindow(), NULL, wc.hInstance, NULL );
	g_hwnd = hWnd;
	...
	delete g_pLog;
	delete g_pCamera;
    UnregisterClass(TEXT("D3D Camera"), wc.hInstance );
    return 0;
}
